...
class FileMixin:
    ...
    # These depend on subclasses providing: multiplicity,
    ...
    def alpha_fmo_gap(self):
        """Returns the frontier molecular orbital (FMO) gap for alpha-spin
        orbitals,
    ...
    def beta_fmo_gap(self):
        """Returns the frontier molecular orbital (FMO) gap for beta-spin
        orbitals,
        for open-shell systems.
...
class GaussianFileMixin(FileMixin):
    ...
    def _get_modred_scan_coords(modred_list_of_string):
        ...
        modred = {}
        coords = []
        # modred = {'num_steps': 10, 'step_size': 0.05, 'coords': [[1, 2], [3,
        # 4]]}
        for raw_line in modred_list_of_string:
            ...
            # obtain num_steps and step_size (assumed the same for each scan
            # coordinate)
...
class ORCAFileMixin(FileMixin):
            ...
            if "%mdci" in line.lower():
                ...
                for next_line in next_lines:
                    if "cutoff" in next_line.lower():
                        # Find the string prior to it. This is assuming the
                        # input is written by
                        # this program where the comment on the cutoff is also
                        # written
    ...
    def solvent_model(self):
        ...
        for i, line in enumerate(self.contents):
            ...
            # solvent specification not in the route string but in the %cpcm
            # block (ORCA_Test_0829.inp)
