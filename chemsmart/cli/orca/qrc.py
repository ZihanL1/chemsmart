...
def qrc(
    ...
    **kwargs,
    ...
    """
    CLI subcommand for running ORCA quick reaction coordinate (QRC)
    calculations.
    ...
    Examples:
        ...
        `chemsmart sub orca -p proj -f ts.log qrc -m 2 -a 1.2` runs QRC
        optimization jobs
        after displacing along vibrational mode 2 (-m 2) by +/- 1.2 Angstroms
        (-a 1.2).

        `chemsmart sub orca -p proj -f ts.log qrc -j ts -m 2 -a 1.5` runs QRC
        TS jobs
        after displacing along vibrational mode 2 (-m 2) by +/- 1.5 Angstroms
        (-a 1.5).

        `chemsmart sub orca -p proj -f ts.log qrc -j modred -c [1,2] -m 2 -a
        1.5`
