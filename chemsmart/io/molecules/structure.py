...
class Molecule:
    ...
    frozen_atoms: list of integers, one for each atom, indicating which atoms
    are frozen.
    ...
    def __init__(
        ...
        # use the relationship between bond orders and bond lengths from J.
        # Phys. Chem. 1959, 63, 8, 1346
        #                 1/Ls^3 : 1/Ld^3 : 1/Lt^3 = 2 : 3 : 4
        # From experimental data, the aromatic bond length in benzene is ~1.39
        # Å,
        # which is between single (C–C, ~1.54 Å) and double (C=C, ~1.34 Å)
        # bonds.
    ...
    def __getitem__(self, idx):
        ...
        Interprets the input idx as 1-based indices by index adjustment (i -
        1).
    ...
    def estimated_dispersion(self):
        ...
        # Use a factor of 1.5 to ensure sufficient dispersion for Voronoi
        # tessellation
    ...
    def voronoi_dirichlet_occupied_volume(self):
        """Calculate the occupied volume of the molecule using
        Voronoi-Dirichlet tessellation.

        Note: This method requires the pyvoro package, which can be installed
        with:
    ...
    def crude_volume_by_atomic_radii(self):
        """Calculate the crude occupied volume of the molecule using atomic
        radii.
        """
    ...
    def crude_volume_by_vdw_radii(self):
        """Calculate the crude occupied volume of the molecule using van der
        Waals radii.
        """
        from chemsmart.utils.geometry import calculate_crude_occupied_volume
    ...
    @property
    def vdw_volume(self):
        """Calculate the occupied volume of the molecule using van der Waals
        radii.
        ...
        See Also
        --------
        grid_vdw_volume : Grid-based volume (more accurate for complex
        molecules)
    ...
    def grid_vdw_volume(self):
        ...
        all orders of atomic overlaps and provides more accurate volume
        estimates
    ...
    def moments_of_inertia_principal_axes(self):
        """
        Obtain moments of inertia along principal axes from molecular
        structure.
        """
    ...
    def _get_moments_of_inertia_weighted_mass(self):
        """
        Calculate the moments of inertia of the molecule. Use natural abundance
        weighted masses.
        Units of amu Å^2.
        """
        if self.num_atoms == 1:
    ...
    def _get_moments_of_inertia_most_abundant_mass(self):
        """
        Calculate the moments of inertia of the molecule. Use most abundant
        masses.
        Units of amu Å^2.
        """
        if self.num_atoms == 1:
    ...
    def get_dihedral(self, idx1, idx2, idx3, idx4):
        """
        Calculate the dihedral angle between four points, about bond formed by
        idx2 and idx3.
    ...
    def from_symbols_and_positions_and_pbc_conditions(
        ...
        """
        Create molecule from symbols, positions and periodic boundary
        conditions.
        """
        return cls(
    ...
    def from_pubchem(cls, identifier, return_list=False):
        ...
        Returns:
            Molecule or list or None: Molecule object from PubChem, None if not
            found

        Raises:
            requests.exceptions.RequestException: For network or HTTP-related
            issues
        """
    ...
    def from_rdkit_mol(cls, rdMol: Chem.Mol) -> "Molecule":
        """
        Creates a Molecule instance from an RDKit Mol object, assuming a single
        conformer.
        """
        if rdMol is None:
        ...
        # Extract atomic positions from the first conformer (assuming single
        # conformer)
    ...
    def write_coordinates(self, f, program=None):
        ...
        Args:
            ...
            program (str, optional): Format to use ('gaussian' or 'orca').
            Default 'gaussian'
    ...
    def _write_orca_coordinates(self, f):
        ...
        # commented above out since with frozen atom or not, the geometry is
    ...
    def _determine_level_from_atom_index(self, atom_index):
        ...
        if self.high_level_atoms is not None:
            if atom_index in self.high_level_atoms:
                ...
                # if high level atoms is given, then low level atoms will be
                # needed
                return "L"
        else:
            return None
    ...
    def to_rdkit(self, add_bonds=True, bond_cutoff_buffer=0.05, adjust_H=True):
        """Convert Molecule object to RDKit Mol with proper stereochemistry
        handling.
        Args:
            add_bonds (bool): Flag to add bonds to molecule or not.
            bond_cutoff_buffer (float): Additional buffer for bond cutoff
            distance.
            From testing, see test_resonance_handling, it seems that a value of
            0.1Å
            works for ozone, acetone, benzene, and probably other molecules,
            too.
        ...
        # I comment the following out since we do not want to modify the
        # molecule
    ...
    def _add_bonds_to_rdkit_mol_vectorized(
        ...
        """
        Add bonds to the RDKit molecule using a vectorized approach to compute
        bond orders.
        """
    ...
    def to_graph(self, bond_cutoff_buffer=0.05, adjust_H=True) -> nx.Graph:
        """
        Convert a Molecule object to a connectivity graph with vectorized
        calculations.
        Bond cutoff value determines the maximum distance between two atoms
        to add a graph edge between them. Bond cutoff is obtained using
        Covalent
    ...
    def vibrationally_displaced(
        ...
        """
        Create a geometry (or trajectory) displaced along a *mass-weighted*
        normal mode.

        Args:
            mode_idx (int): Mode index (1-based, negatives allowed)
            amp (float): Target maximum atomic displacement in Å after
            normalization.
            nframes (int | None): If provided, generate that many frames over
            one period
            ...
            normalize (bool): If True, scale the (un-weighted) mode so its
            largest
                per-atom displacement is 1.0, making `amp` the max
                displacement.
            return_xyz (bool): If True and `nframes` is set, return a
    ...
    def xyz_to_pdb(
        ...
        Args:
            ...
            mode (str): File mode passed to ``write_xyz`` when creating the XYZ
            file.
...
class CoordinateBlock:
    ...
    def constrained_atoms(self):
        """
        Returns a list of constraints in Gaussian format where 0 means
        unconstrained
    ...
    def convert_coordinate_block_list_to_molecule(self):
        """
        Function to convert coordinate block supplied as text or as a list of
        lines into
        Molecule class.
        """
    ...
    def _get_symbols(self):
        ...
        for line in self.coordinate_block:
            ...
            # f'The geometry specification, `Symbol x y z` line should have 4
            # members \n'
            # f'but is {len(line_elements)} instead!')
            # not true for some cubes where the atomic number is repeated as a
            # float:
            ...
            if all(el.isdigit() for el in line_elements):
                # skip the charge and multiplicity line of QM/MM coordinate
                # block
    ...
    def _get_atomic_numbers_positions_and_constraints(self):
        ...
        for line in self.coordinate_block:
            ...
            # Decide how to interpret the second token: constraint flag vs
            # coordinate
            try:
            ...
            # If the last token is non-numeric (e.g., partition label like
            # H/M/L),
            # we should not attempt to treat the second token as a constraint
            # flag.
            ...
            if len(line_elements) > 4:
                if is_constraint_flag and last_token_numeric:
                    # Frozen coordinate line: second token is an explicit -1/0
                    # flag
                    ...
                    # Cube file style where the atomic number is repeated as
                    # float
                    x_coordinate = float(line_elements[2])
                    y_coordinate = float(line_elements[3])
                    z_coordinate = float(line_elements[4])
                else:
                    # Standard coordinate line (including cases like trailing
                    # partition labels)
...
class QMMMMolecule(Molecule):
    """
    Standardise QMMM-related objects subclass normal objects (settings,
    jobrunner, molecule, etc),
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if molecule is not None:
            # inherit all parameters from the molecule object including class
            # methods
            sig = inspect.signature(Molecule.__init__)
            valid_params = set(sig.parameters.keys()) - {"self"}

            # Keep only attributes of molecule that are valid init params and
            # override with any explicit kwargs if given
            init_params = {
    ...
    def _get_partition_levels(self):
        """Obtain the list of partition levels for the atoms in the system.
        Returns:
            list: List of partition levels as strings (H, M, L) for the atoms
            in the system.
        """
        # convert atom indices to lists if they are not already so
        # for example high_level_atoms=[[18-28], [29-39], [40-50], [51-61],
        # [62-72]],
        ...
        if (len(set_h) + len(set_m) + len(set_l)) != int(
            ...
            # allow the case where user only supplied some layers and intended
            # others empty
            # but raise if they provided explicit low/medium/high that don't
            # cover all atoms
