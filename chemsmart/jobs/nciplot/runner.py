...
class NCIPLOTJobRunner(JobRunner):
    ...
    def __init__(
        ...
        Args:
            ...
            scratch: Whether to use scratch directory (defaults to class
            SCRATCH)
    ...
    def _postrun(self, job):
        ...
        if self.scratch:
            # If job was run in scratch, copy files to job folder except temp
            # files
...
class FakeNCIPLOTJobRunner(NCIPLOTJobRunner):
    ...
    Attributes:
        ...
        server: Server configuration used for execution (retained/used
        normally).
        scratch (bool): Whether scratch is enabled for this runner.
        scratch_dir (str): Path to scratch directory, if used.
        num_cores (int): Number of CPU cores allocated (from server).
...
class FakeNCIPLOT:
    ...
    Attributes:
        ...
        output_filepath (str): Full path to the simulated output file
        (.nciout).
    """
