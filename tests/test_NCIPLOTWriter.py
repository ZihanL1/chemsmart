...
class TestNCIPLOTInputWriter:
    ...
    def test_write_nci_promolecular_with_scratch(
        ...
        # Simulate the runner's prerun which creates scratch dir and copies
        # files
...
class TestNCIPLOTPathHandling:
    ...
    def test_write_nci_with_full_path_in_scratch(
        ...
        # Create a source directory to simulate files being in a different
        # location
        ...
        # Check that the filename written is just the basename, not the full
        # path
        with open(nci_file, "r") as f:
            lines = f.readlines()
        assert lines[0] == "1\n"
    ...
    def test_write_nci_converted_file_with_full_path(
        ...
        # Create a minimal fake log file (won't actually be converted in this
        # test)
