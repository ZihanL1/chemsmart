...
class Gaussian16Output(GaussianFileMixin):
    ...
    def multiplicity(self):
        ...
        for line in self.contents:
            if "Charge" in line and "Multiplicity" in line:
                ...
                # # Charge = 1 Multiplicity = 2 for low level calculation on
                # real system.
    ...
    def input_coordinates_block(self):
        """Obtain the coordinate block from the input that is printed in the
        outputfile.
        """
                ...
                for j_line in self.contents[i + 2 :]:
                    ...
                    if j_line.split()[0] not in p.PERIODIC_TABLE:
                        if j_line.startswith("Charge ="):
                            ...
                            # e.g., in QM/MM output files, the first element is
                            # not the coordinates information
                            # e.g., "Charge = 1 Multiplicity = 2 for low level
                            # calculation on real system."
                            continue
                        # elif j_line.startswith("TV"): we still add the line
                        # for PBC
    ...
    def all_structures(self):
        ...
        Use Standard orientations to get the structures; if not, use input
        orientations.
    ...
    def _get_all_molecular_structures(self):
        """
        Build and return the list of Molecule objects parsed from a
        calculation.
        ...
        Special handling (as per design):
          - Non-link & normal termination: de-duplicate the terminal (e.g.
          freq) frame.
          ...
          - Link & abnormal termination:
              * if multiple frames: drop the first (carry-over), then use
              safe_min_lengths.
        ...
        def drop_first():
            ...
            if energies:
                energies = energies[1:]
            # Forces do not need to be dropped here because no force
            # computation occurs at the first link job.
            # This is intentional: the forces array is already aligned with the
        ...
        # Right-trim auxiliaries to the number of orientations (no data loss in
        # orientations)
        ...
        if self.is_link:
            if self.normal_termination:
                ...
                if len(orientations) > 1:
                    # Multiple frames: drop carried-over first, then treat as
                    # abnormal
                    drop_first()
                    # Fall through to abnormal handling below
                    # (safe_min_lengths)
                else:
        ...
        # 3) De-dup only for normal-termination paths (incl. link-normal after
        # drop)
        if self.normal_termination:
        ...
        # 4) Compute safe min length for logging and for abnormal (non-link or
        # link>1-after-drop)
        ...
        if self.normal_termination:
            ...
            # Abnormal (non-link, or link with >1 frame after drop): truncate
            # safely
    ...
    ###### the following properties relate to intermediate geometry
    ###### optimizations
    ...
    def gen_genecp(self):
        """
        String specifying if gen or genecp is used in the calculation output
        file.
        """
    ...
    def reduced_masses(self):
        """
        Obtain list of reduced masses corresponding to the vibrational
        frequency.
        """
        reduced_masses = []
        for line in self.contents:
    ...
    def force_constants(self):
        """
        Obtain list of force constants corresponding to the vibrational
        frequency.
        """
        force_constants = []
        for line in self.contents:
    ...
    def ir_intensities(self):
        """
        Obtain list of IR intensities corresponding to the vibrational
        frequency.
        """
        IR_intensities = []
        for line in self.contents:
    ...
    def vibrational_mode_symmetries(self):
        """
        Obtain list of vibrational mode symmetries corresponding to the
    ...
    def frozen_atoms_masks(self):
        ...
        using precomputed frozen_coordinate_indices and
        free_coordinate_indices.
        """
    ...
    def scf_energies(self):
        """
        Obtain SCF energies from the Gaussian output file. Default units of
        Hartree.
        """
        scf_energies = []
        for line in self.contents:
    ...
    def mp2_energies(self):
        """
        Obtain MP2 energies from the Gaussian output file. Default units of
        Hartree.
        """
        mp2_energies = []
        for line in self.contents:
    ...
    def oniom_energies(self):
        """
        Obtain ONIOM energies from the Gaussian output file. Default units of
        Hartree.
        """
        oniom_energies = []
        for line in self.contents:
    ...
    def oniom_layer_energies(self):
        """Obtain ONIOM energies from the Gaussian output file. Default units
        of Hartree.
        """
        layer_energies = {}
        for line in self.contents:
    ...
    def input_orientations_pbc(self):
        """Obtain structures in Input Orientation with PBC from Gaussian output
        file.
        """
    ...
    def standard_orientations_pbc(self):
        """Obtain structures in Standard Orientation with PBC from Gaussian
        output file.
        """
    ...
    def excitation_energies_eV(self):
        """
        Read TDDFT transitions and return the transition energies in eV as a
        list.
        """
    ...
    def absorptions_in_nm(self):
        """
        Read TDDFT transitions and return the absorbed wavelengths in nm as a
        list.
        """
        absorptions_in_nm = []
        for i in self.tddft_transitions:
    ...
    def _read_transitions_and_contribution_coefficients(self):
        ...
        for i, line in enumerate(self.contents):
            if line.startswith("Excited State"):
                ...
                # parse the lines that follow until an empty line is
                # encountered
                j = 1
    ...
    def alpha_occ_eigenvalues(self):
        """
        Obtain all eigenenergies of the alpha occupied orbitals and convert to
        eV.
        """
        alpha_occ_eigenvalues = []

        # Iterate through lines in reverse to find the last block of
        # eigenvalues
        eigenvalue_blocks = []
        current_block = []
    ...
    def _get_hirshfeld_charges_spins_dipoles_cm5(self):
        """
        Obtain Hirshfeld charges, spin densities, dipoles, and CM5 charges from
        the output file.
        """
    ...
    def _get_hirshfeld_charges_spin_densities_cm5_charges_heavy_atoms(self):
        """
        Obtain Hirshfeld charges, spin densities and CM5 with hydrogens summed
        into heavy atoms.
        """
    ...
    def moments_of_inertia(self):
        """
        Obtain moments of inertia from the output file which are in atomic
        units
    ...
    def to_dataset(self, **kwargs):
        """
        Convert Gaussian .log file to Dataset with all data points taken from
        the .log file.
    ...
    def oniom_partition(self):
        ...
        for i, line in enumerate(self.contents):
            if "Symbolic Z-matrix:" in line:
                # First ONIOM format: coordinates start 4 lines after the
                # header
                if "Charge" not in self.contents[i + 4]:
                    ...
                    # Alternative ONIOM format: coordinates start 7 lines after
                    # the header
                    atom_index = 1
    ...
    def oniom_get_charge_and_multiplicity(self):
        """Obtain the charge and multiplicity of the system in the ONIOM
        calculation.
        Returns:
...
class Gaussian16WBIOutput(Gaussian16Output):
            ...
            if (
                ...
                for j_line in self.contents[i + 2 :]:
                    ...
                    if len(j_line) != 0:
                        ...
                        # Construct the sub-key for each NAO entry, e.g.,
                        # "NAO1", "NAO2", etc.
    ...
    def total_electrons(self):
        """
        Get the total number of electrons corresponding to each atom as a
        dictionary.
        """
        total_electrons = {}
        for atom_key, atom_data in self.natural_population_analysis.items():
    ...
    def electronic_configuration(self):
        """
        Get electronic configuration for each atom and store results in a
        dictionary.
        """
...
class Gaussian16OutputWithPBC(Gaussian16Output):
    """
    class for parsing and obtaining information from Gaussian output file with
    PBC.
    """
