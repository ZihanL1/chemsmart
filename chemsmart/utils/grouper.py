...
class MoleculeGrouper(ABC):
    ...
    def _validate_inputs(self) -> None:
        ...
        Raises:
            TypeError: If molecules is not iterable or contains non-Molecule
            items.
        """
...
class RMSDGrouperSharedMemory(MoleculeGrouper):
    ...
    def group(self) -> Tuple[List[List[Molecule]], List[List[int]]]:
        ...
        # 🏃‍♂️ **2️⃣ Run Parallel RMSD Calculation Using Explicit Shared
        # Memory**
        indices = [(i, j) for i in range(n) for j in range(i + 1, n)]
...
class TanimotoSimilarityGrouper(MoleculeGrouper):
    """
    Groups molecules based on fingerprint similarity using Tanimoto
    coefficient.

    This class supports different fingerprint types and uses connected
    components
    clustering to group structurally similar molecules.

    Tanimoto similarity is a measure of how similar two molecular fingerprints
    are,
    ...
    0.95 - 1.0	Very strict: Only almost identical molecules are grouped.
                            Ideal for highly similar molecules (e.g.,
                            stereoisomers).
    0.80 - 0.95	Moderately strict: Groups structurally similar molecules.
                            Useful for clustering molecules with minor
                            functional group differences.
    0.50 - 0.80 More relaxed: Groups molecules with broad structural
    similarities.
                            Good for structural analogs or scaffold-based
                            grouping.
    < 0.50	Very lenient: Even molecules with weak similarity are grouped.
                            Not recommended unless looking for very broad
                            chemical families.
    """
...
class RDKitIsomorphismGrouper(MoleculeGrouper):
    ...
    def _check_isomorphism(self, mol1: Molecule, mol2: Molecule) -> bool:
        ...
        Returns:
            bool: True if InChIKeys match; False otherwise (including on
            failure).
        """
        try:
...
class ConnectivityGrouper(MoleculeGrouper):
    ...
    def __init__(
        ...
        Args:
            molecules (Iterable[Molecule]): Collection of molecules to group.
            num_procs (int): Number of processes for parallel computation.
            threshold (float): Buffer for bond cutoff distance. Defaults to
            0.0.
            adjust_H (bool): Whether to adjust hydrogen bond detection.
                Defaults to True.
        """
