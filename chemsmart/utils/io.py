...
def create_molecule_list(
    ...
    Notes:
        - `orientations_pbc`, `energies`, and `forces` are indexed by
        structure;
...
def replace_word(text, old_word, new_word, case_sensitive=True):
    ...
    Example:
        >>> replace_word("gen test
        noeigentest","gen","def2svp",case_sensitive=True)
...
def match_outfile_pattern(line) -> str | None:
    ...
    Returns:
        str | None: Program name ("gaussian", "orca", "xtb", "crest") if
        matched, else None.
    """
    patterns = {
        "crest": [
...
def find_output_files_in_directory(directory, program):
    ...
    Args:
        ...
        program (str): Target QC program, e.g., "gaussian", "orca", "xtb",
        "crest".
...
def load_molecules_from_paths(
    ...
    """
    Load molecules from a list of file paths, assigning unique names to each
    molecule.

    For each file in `file_paths`, this function loads one or more molecular
    structures
    using `Molecule.from_filepath`, assigns a unique name to each molecule
    based on the
    file name and structure index, and returns a list of all loaded molecules.

    Args:
        file_paths (list of str or Path): List of file paths to load molecules
        from.
        index (int or str or None): Index or slice to select specific
        structures from each file.
            If None, defaults to "-1" (last structure).
        add_index_suffix_for_single (bool, optional): If True, appends an index
        suffix to
            the molecule name even if only a single structure is loaded from a
            file.
        check_exists (bool, optional): If True, checks that each file exists
        before loading.

    Returns:
        list of Molecule: List of loaded Molecule objects, each with a unique
        name.
    ...
    for i, file_path in enumerate(file_paths):
        ...
        try:
            ...
            # assign unique names per-structure when file contains multiple
            # structures
            ...
            if isinstance(mols, list) and len(mols) > 1:
                ...
                # Optional suffix for single-structure files (filenames
                # branch).
...
def select_items_by_index(
    ...
    Args:
        ...
        index_spec (int or str or slice or None): Index specification for
        selection.
            ...
            If str: String specification (1-based indexing, parsed by
    ...
    Note:
        ...
        - slice indices never raise errors; they return empty or partial
        results.
...
def convert_string_indices_to_pymol_id_indices(string_indices: str) -> str:
    """
    Convert a comma-separated list of atom index ranges into a PyMOL `id`
    selection.

    The input is expected to be a string of indices and/or index ranges
    separated
    by commas, e.g.:
    ...
    So any atom that satisfies any one of those conditions is included in the
    selection.
    ...
    ------
    ValueError
        If the input string is empty or contains no valid indices after
        stripping.
    """
