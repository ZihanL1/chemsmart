...
class GaussianJobSettings(MolecularJobSettings):
    ...
    def merge(
        ...
        Args:
            keywords (list): Specific list of keywords to merge.
                ...
                If None, all settings will be merged (Caution: may cause issue
                if e.g.,
                genecp log file used to prepare input without genecp).
            other (JobSettings, dict): Settings to merge. Can also take the
    ...
    def determine_basis_keyword(self, molecule):
        """
        Determine the appropriate basis keyword (gen or genecp) for the
        molecule.
...
class GaussianIRCJobSettings(GaussianJobSettings):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            direction (str, optional): IRC direction ('forward'/'reverse'). If
            None, run both directions.
    ...
    def _get_route_string_from_jobtype(self):
        ...
        if self.additional_route_parameters is not None:
            # Check if the additional parameters are already in the route
            # string
...
class GaussianLinkJobSettings(GaussianJobSettings):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            recorrect (str, optional): IRC recorrection strategy for IRC link
            jobs.
    ...
    def _get_link_route_string_from_jobtype(self):
        ...
        reading specifications. Handles IRC jobs by using
        ...
        if self.jobtype in ["ircf", "ircr"]:
            # Create a temporary GaussianIRCJobSettings instance with current
            # settings
...
class GaussianQMMMJobSettings(GaussianJobSettings):
    """
    Configuration settings for Gaussian QM/MM calculations using ONIOM
    methodology.

    This class manages all parameters needed to set up multi-layer ONIOM
    calculations
    in Gaussian, which partition molecular systems into different regions
    treated with
    varying levels of theory. The ONIOM approach enables accurate quantum
    mechanical
    treatment of chemically active regions while efficiently handling large
    molecular
    ...
    **3-Layer ONIOM**: High(QM):Medium(QM):Low(MM)
    ...
    Attributes:
        jobtype (str, optional): Type of ONIOM calculation ('sp', 'opt',
        'freq', 'ts', 'irc',
            'modred', 'scan'). When using CLI commands, this is automatically
            inferred from
            the parent command (e.g., 'chemsmart sub gaussian opt qmmm' sets
            jobtype='opt').

        Level-specific theory parameters:
            high_level_functional (str): DFT functional for high layer (e.g.,
            'B3LYP', 'M06-2X')
            high_level_basis (str): Basis set for high layer (e.g., '6-31G*',
            'def2-TZVP')
        ...
        Charge and multiplicity specifications:
            charge_total/mult_total (int): Full system properties (legacy:
            charge_total/real_multiplicity)
            charge_intermediate/mult_intermediate (int): Intermediate system
            properties (legacy: int_charge/int_multiplicity)
            charge_high/mult_high (int): Model/high-layer properties (legacy:
            model_charge/model_multiplicity)
    ...
    Note:
        The real/intermediate/model charge and multiplicity specifications
        follow
        ...
        ratios if not specified. The format is {(atom1,atom2):
        [low,medium,high]}.
        ...
        **kwargs,
        ...
        Args:
            jobtype (str, optional): Type of ONIOM calculation to perform.
                Options: 'sp' (single-point), 'opt' (optimization), 'freq'
                (frequency),
                'ts' (transition state), 'irc' (intrinsic reaction coordinate),
                'modred' (redundant coordinates), 'scan' (coordinate scan)

                Note: When using the CLI with `chemsmart sub gaussian <jobtype>
                qmmm`,
                the jobtype is automatically inferred from the parent command
                and does
            ...
            Theory level parameters:
                ...
                high_level_force_field (str): Force field for high MM layer
                (rare)
                medium_level_functional (str): DFT functional for medium QM
                layer
                medium_level_basis (str): Basis set for medium QM layer
                medium_level_force_field (str): Force field for medium MM layer
                low_level_functional (str): DFT functional for low QM layer
                (uncommon)
            ...
            Charge and multiplicity:
                charge_total (int): Total charge of complete molecular system
                (legacy: charge_total)
                mult_total (int): Spin multiplicity of complete system (legacy:
                real_multiplicity)
                charge_intermediate (int): Charge of high+medium layers
                (legacy: int_charge)
                mult_intermediate (int): Multiplicity of high+medium layers
                (legacy: int_multiplicity)
                charge_high (int): Charge of high layer only (legacy:
                model_charge)
                mult_high (int): Multiplicity of high layer only (legacy:
                model_multiplicity)
            ...
            Atom partitioning:
                ...
                medium_level_atoms (list/str): Atoms in medium layer (3-layer
                only)
                low_level_atoms (list/str): Atoms in low layer (usually
                auto-assigned)
                bonded_atoms (list): Covalent bonds crossing layer boundaries.
                    Format: [(atom1, atom2), (atom3, atom4)] where atoms are
                    1-indexed
        ...
        Note:
            ...
            - If only 2 layers specified, use high_level_* and low_level_*
            parameters
            - Force fields must be available in your Gaussian installation
            - Link atoms are automatically placed for bonded_atoms
            specifications
            - The parent class 'functional' and 'basis' attributes are set to
            high_level values
        """
        super().__init__(**kwargs)
        ...
        # Canonical charge/multiplicity names (aligned with ORCA) with legacy
        # fallbacks
    ...
    def validate_and_assign_level(
        ...
        if force_field:
            ...
            # if force field is not given, then functional and basis can be
            # given,
            # so that level of theory takes functional and basis set
            if functional and basis:
                level_of_theory = f"{functional}/{basis}"
            else:
                # but functional and basis set can also not be given, in which
                # case,
                # all 3 are None and overall level of theory for that layer is
                # None.
    ...
    def _get_route_string_from_jobtype(self):
        """Generate QM/MM route string with job type, freq, and ONIOM
        specification.
        """
        route_string = "#"
        if self.dieze_tag:
    ...
    def _get_charge_and_multiplicity(self):
        ...
        When only a single value pair is specified, all levels will use those
        values.
        If two pairs of values are included, then third pair defaults to same
        values as second pair.
        If final pair is omitted for an S-value job, it defaults to values for
        the real system at low level.
        For such two-layer ONIOM jobs, users are required to specify the charge
        and multiplicity of high-level
         layer and low-level layer, instead high and medium level.

        For 3-layers ONIOM, the format is:
        cRealL sRealL [cIntM sIntM [cIntL sIntL [cModH sModH [cModM sModM
        [cModL sModL]]]]]
        Real, Int=Intermediate system, and Mod=Model system, and second
        character
        ...
        if (
            ...
            # Defaults for missing charge / spin multiplicity pairs are taken
            # from the next highest
            # calculation level and / or system size.
            if all(var is None for var in charge_and_multiplicity_list[2:]):
                # only charge and multiplicity of real system is specified,
                # so the charge and multiplicity of other systems will be the
                # same as the real system
                for i in range(2, len(charge_and_multiplicity_list), 2):
                    charge_and_multiplicity_list[i] = real_low_charge
                    charge_and_multiplicity_list[i + 1] = real_low_multiplicity
            elif all(var is None for var in charge_and_multiplicity_list[4:]):
                # only charge and multiplicity of real system and that of
                # intermediate layer,
                # medium level-of-theory are specified, the charge and
                ...
                # only charge and multiplicity of real system, intermediate
                # layer, medium level-of-theory
                # and intermediate layer, medium level-of-theory are specified,
                # the charge and multiplicity of other
                # systems will be the same as intermediate layer, medium
                # level-of-theory,...
                for i in range(6, len(charge_and_multiplicity_list), 2):
                    charge_and_multiplicity_list[i] = int_med_charge
                    charge_and_multiplicity_list[i + 1] = int_med_multiplicity
            elif all(var is None for var in charge_and_multiplicity_list[8:]):
                # the rest systems will follow the model system, high
                # level-of-theory
    ...
    def __eq__(self, other):
        ...
        Compares all attributes between two QMMM settings objects, including
        the
