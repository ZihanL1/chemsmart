...
class ORCAOutput(ORCAFileMixin):
    ...
    This class provides comprehensive parsing capabilities for ORCA output
    files,
    ...
    def _get_input_structure_coordinates_block_in_output(self):
        """In ORCA output file, the input structure is rewritten and for single
        points,
        is same as the output structure.

        An example of the relevant part of the output describing the structure
        is:
        | 20> * xyz 0 1
        ...
        | 24> *.
        # this will not work if the input file is supplied separately as .xyz
        # file
        """
    ...
    def _get_first_structure_coordinates_block_in_output(self):
        ...
        for line in self.contents:
            ...
            if found_header:
                match = pattern.match(line)
                if match:
                    # Extract the last 4 elements (symbol, x, y, z) and join
                    # with double spaces
    ...
    def _get_constraints(self):
        ...
        Reads from Redundant Internal Coordinates block where, if DOF is
        constrained,
        there is a "C" at the end of the line.
        Returns:
             a dict similar to optimized parameters, specifying the constraints
             and
    ...
    def optimized_output_lines(self):
        """Chunk of outputfile where the properties are calculated based on the
    ...
    def scf_convergence(self):
        ...
        for line in self.contents:
            if pattern.search(line):
                # return string directly after 'convergence' -- convergence
                # criteria
                # ['sloppy', 'loose', 'medium', 'strong', 'tight', 'verytight',
                # 'extreme']
    ...
    def all_structures(self):
        """Obtain all structures in ORCA output file, including intermediate
        points if present.
        ...
        # Clean duplicate structures (e.g., last structure might repeat in some
        # cases)
        ...
        if self.normal_termination:
            ...
            # For abnormal termination, exclude the last structure (likely
            # incomplete)
    ...
    def get_optimized_parameters(self):
        ...
        ## the above result will return a dictionary storing the optimized
        ## parameters:
        ## optimized_geometry = { b(h1,o0) : 0.9627, b(h2,o0) : 0.9627,
        ## a(h1,o0, h2) : 103.35 }
    ...
    def _get_optimized_final_structure(self):
        """Obtain the final optimized structure from ORCA geometry optimization
        job.
        ...
        if len(self.optimized_output_lines) != 0:
            for i, line_i in enumerate(self.optimized_output_lines):
                if "FINAL ENERGY EVALUATION AT THE STATIONARY POINT" in line_i:
                    # only start getting the structures that appear after this
                    # line (stationary point)
                    for line_j in self.optimized_output_lines[i + 4 :]:
                        if len(line_j) == 0:
                            # stop when an empty line appears, else it will
                            # continue parsing non-geometry
                            break
    ...
    def last_structure(self):
        """
        Return last structure, whether the output file has completed
        successfully or not.
        """
        return self.all_structures[-1]
    ...
    def _get_molecule_from_sp_output_file(self):
        ...
        for line in self.contents:
            if "coordinates will be read from file:" in line:
                ...
                # If molecule is not found, get it from the input lines in the
                # output file
    ...
    def _get_orbital_energies_and_occupancy(self):
        ...
        for line in self._get_last_orbital_energies_section()[2:]:
            # ignore the lines '----------------' and one empty line that
            # follows
            line_elements = line.split()
    ...
    def is_unrestricted(self):
        ...
        Returns True if the calculation used separate alpha and beta spin
        orbitals.
        """
    ...
    def vibrational_frequencies(self):
        """Return vibrational frequencies without translational and rotational
        modes.
        """
            ...
            if line_i == "NORMAL MODES":
                ...
                while j < len(self.optimized_output_lines):
                    ...
                    if line_of_all_integers(j_line):
                        ...
                        # Read the next 3*num_atoms lines (x, y, z for each
                        # atom)
                        ...
                        # Convert to numpy array: shape (3N,
                        # num_modes_in_block)
                        ...
                        for mode_col in range(num_modes_in_block):
                            ...
                            # Reshape to (num_atoms, 3) by grouping every 3
                            # consecutive values
    ...
    def internal_energy(self):
        ...
        E(el) = E(kin-el) + E(nuc-el) + E(el-el) + E(nuc-nuc) is the total
        energy from the electronic structure
            calculation
        E(ZPE) - the the zero temperature vibrational energy from the frequency
        calculation
        E(vib) - the the finite temperature correction to E(ZPE) due to
        population of excited vibrational states
    ...
    def zero_point_energy(self):
        """E(ZPE) - the the zero temperature vibrational energy from the
    ...
    def total_thermal_correction_due_to_trans_rot_vib(self):
        """
        Get total thermal correction due to translation, rotation and
        vibration.
        """
        return (
    ...
    def _get_gradient(self):
        ...
        for i, line in enumerate(self.contents):
            if "current gradient" in line:
                # check 3N + 3 lines following the match, where N is number of
                # atoms
                grad_data = []
