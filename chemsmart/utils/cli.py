    ...
    def argument_keyword(arg, value, is_flag, secondary_opts):
        ...
        if value is False:
            if is_flag and len(secondary_opts) == 0:
                # Click "is_flag" option. Click param class has no way to check
                # if
                # flag is True or not only can determine by checking
                # arg_secondary_opts
                return ""
...
def get_setting_from_jobtype_for_gaussian(
    ...
    Raises:
        ...
        AssertionError: If required parameters are missing for specific job
        types.
    """
    if jobtype is None:
...
def check_scan_parameters_consistency_orca(
    ...
    """
    Validate consistency between scan coordinates and their parameters for
    ORCA.
    ...
    Args:
        coords: Coordinate specification - list[int] for single coordinate
                or list[list[int]] for multiple coordinates.
        dist_start: List of starting distances - must match number of
        coordinates.
    ...
    Examples:
        ...
        # Valid multiple coordinates
        check_scan_parameters_consistency_orca([[1,2],[3,4]], [3.0,2.5],
        [1.2,1.0], [10,15])

        # Invalid - mismatched counts
        check_scan_parameters_consistency_orca([[1,2],[3,4]], [3.0], [1.2,1.0],
        [10,15])
        # Raises ValueError
    """
    if isinstance(coords[0], list):
