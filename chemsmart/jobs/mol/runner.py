...
class PyMOLJobRunner(JobRunner):
    ...
    def _create_process(self, job, command, env, append_mode=False):
        ...
        Args:
            ...
            append_mode: If True, append to existing log/err files instead of
            overwriting.
                         Used for batch processing to preserve logs from
                         previous batches.
...
class PyMOLHybridVisualizationJobRunner(PyMOLVisualizationJobRunner):
    ...
    This job runner supports an arbitrary number of groups, determined
    dynamically.
    """
    ...
    def _write_hybrid_pml(self, job):
        ...
        Creates a PyMOL script file that sets up hybrid visualization style
        with appropriate
    ...
    def _write_default_pymol_style(self, job, f):
        """Write the pymol style without settings for stick color to the pml
        file.
        """
    ...
    def _write_faded_colors(self, job, f):
        """Write the faded colors for non-highlighted C, N, O, P in background
        to the pml file.
        """
        new_color_carbon = (
    ...
    def _write_highlighted_colors(self, job, f):
        """
        Write PyMOL commands to highlight and color the defined groups in a
        job.

        Args:
            job: PyMOLHybridVisualizationJob instance containing group and
            color info.
            f: File object to write the PyMOL commands.

        Behavior:
            - Groups are selected using their atom indices (via _get_groups).
            - Colors are applied according to the user's specification; if no
            color
        ...
        # Retrieve the stick radius from the job or use the default value
        # (0.25)
...
class PyMOLMovieJobRunner(PyMOLVisualizationJobRunner):
    ...
    def _create_movie(self, job, framerate=30, constant_rate_factor=18):
        ...
        Args:
            ...
            constant_rate_factor (int): FFmpeg CRF value (0–51, lower =
            better).
...
class PyMOLAlignJobRunner(PyMOLJobRunner):
    ...
    def run(self, job, **kwargs):
        ...
        # Execute preprocessing and input preparation first to set batch
        # processing flag
        self._prerun(job)
        ...
        if hasattr(job, "use_batch_processing") and job.use_batch_processing:
            ...
            # Use original single batch processing - execute all steps to
            # ensure consistency with original behavior
            logger.info(
    ...
    def _execute_batch(self, job, batch_idx, batch_paths, batch_names):
        ...
        # All batches need to add style script to ensure pymol_style function
        # is available
        ...
        if batch_idx == 0:
            ...
            # Alignment commands - align all to GLOBAL first molecule (not
            # batch first molecule)
            ...
            # Subsequent batches: load new molecules, apply style, and align to
            # global first molecule
