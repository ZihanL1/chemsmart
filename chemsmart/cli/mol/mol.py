...
def mol(
    ...
    """
    CLI subcommand for running PyMOL visualization jobs using the chemsmart
    framework.
    ...
    # Normalize empty tuple to None (click's multiple=True returns () when no
    # -f provided)
    if not filenames:
        filenames = None
    ...
    if filenames:
        # Check if this is an align task by looking for " align" in invokded
        # subcommand
