...
class XYZFile(FileMixin):
    ...
    def get_molecules(self, index=":", return_list=False):
        ...
        Returns:
            Molecule or list: Single molecule or list of molecules with
            energies
        """
        # Ensure that when return_list=False, molecules is always treated as a
        ...
        if len(comments) != 0:
            for i, comment in enumerate(comments):
                ...
                # example case 1: "Empirical formula: C191H241Cu2N59O96P14
                # Energy(Hartree): -25900.214629"
                # energy will be -25900.214629.
                match = re.findall(raw_energy_value_pattern, comment)
                if match:
                    molecules[i].energy = float(match[0])
                    # Assign energy to the only or the first negative float
                    # number
                else:
