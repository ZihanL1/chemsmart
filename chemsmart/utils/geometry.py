...
# Mass parameter is a list of atomic masses corresponding to each coordinate in
# coords."""
...
# moments_of_inertia_principal_axes = np.linalg.eigvalsh(moi_tensor) # Sorted
# eigenvalues
#
...
#
#     Returns:
# - moments_of_inertia_principal_axes (np.array): Sorted eigenvalues of
# moi_tensor.
...
# # Instead of returning evecs.T as in ASE, we return evecs directly, as in
# Gaussian
...
def calculate_crude_occupied_volume(coords, radii):
    ...
    Ignores overlaps between atoms and gives an upper bound to true occupied
    volumes.
    """
    coords = np.array(coords)
    radii = np.array(radii)
...
def calculate_vdw_volume(coords, radii):
    ...
    spherical cap volumes:
        ...
        V_overlap = π(r_i + r_j - d)² × [d² + 2d(r_i + r_j) - 3(r_i - r_j)²] /
        (12d)
        ...
        for j in range(i + 1, len(coords)):
            ...
            if d < sum_radii and d > 0:
                ...
                if d <= abs(diff_radii):
                    ...
                    # Partial overlap: use exact lens-shaped intersection
                    # formula
                    # V = π(r_i + r_j - d)² × [d² + 2d(r_i + r_j) - 3(r_i -
                    # r_j)²] / (12d)
                    overlap = (
                        math.pi
                        * (sum_radii - d) ** 2
...
def calculate_grid_vdw_volume(coords, radii, grid_spacing=0.2):
    ...
    This implementation is similar in concept to RDKit's
