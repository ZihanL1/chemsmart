...
class GaussianRoute:
    ...
    def get_functional_and_basis(self):
        ...
        for each_input in self.route_inputs:
            ...
            if "/" in each_input:
                ...
                # pbepbe 6-31g(d,p)/auto force scrf=(dipole,solvent=water)
                # pbc=gammaonly'
                if len(func_basis) == 2:
            ...
            if "opt" in each_input:
                ...
                if len(opt_route) != 0:
                    ...
                    for opt_option in opt_options:
                        ...
                        # additional_opt_options.append(opt_option) # add
                        # `no/eigentest` only for ts jobs
                        # <-- `eigentest` already included in writing in
                        # GaussianSettings.write_gaussian_input()
                        if any(
    ...
    def get_solvent_model(self):
        ...
        if "scrf" in self.route_string:
            ...
            # if none of the models are present, set default model to pcm as in
            # Gaussian
            if all(
