...
class TestThermochemistry:
    def test_thermochemistry_from_gaussian_output(
        ...
        """Values from Goodvices, as a reference:
           ...
           ********************************************************************
        o nhc_neutral_singlet -1864.040180 0.284336 -1863.732135 0.079236
        0.072784 -1863.811371 -1863.804919
        """
        ...
        # test moments of inertia from molecular structure and from Gaussian
        # output
        ...
        # test moments of inertia principal axes from molecular structure and
        ...
        # test rotational temperatures from molecule (direct calc) same as from
        # Gaussian output
        assert np.allclose(
...
class TestThermochemistryCO2:
    """CO2 is used as an example to test the thermochemical properties of
    linear molecules.
    """
    ...
    def test_thermochemistry_co2_gaussian_output(
        ...
        # components X and Y are swapped but they are physically the same (same
        ...
        # For the zero reference point is the first vibrational energy level
        # (V=0)
        ...
        # S_v = R * Σ((Θ_v,K / T) / (exp(Θ_v,K / T) - 1) - ln(1 - exp(-Θ_v,K /
        # T)))
        ...
        # C_v = R * Σ(exp(-Θ_v,K / T) * ((Θ_v,K / T) / (exp(-Θ_v,K / T) -
        # 1))^2)
    ...
    def test_thermochemistry_co2_qrrho(self, gaussian_co2_opt_outfile):
        """Values from Goodvibes, as a reference:
                goodvibes -f 100 -c 1.0 -t 298.15 --qs grimme --bav "conf"
                co2.log
        Structure E ZPE H T.S T.qh-S G(T) qh-G(T)
           ********************************************************************
        o co2 -188.444680 0.011776 -188.429325 0.021262 0.021262 -188.450587
        -188.450588
           ********************************************************************
        """
        assert os.path.exists(gaussian_co2_opt_outfile)
        ...
        # we got S_R,K = [4.13984132, 4.13984132, 1.00676497, -1.39084927] in J
        # mol^-1 K^-1
        ...
        # S^rrho_v,K = R * [(Θ_v,K / T) / (exp(Θ_v,K / T) - 1) - ln(1 -
        # exp(-Θ_v,K / T))]
        # Θ_v,K = h * v_K / k_B
        expected_theta = (
            6.62606957
        ...
        # we got [1.53092635e+00, 1.53092635e+00, 7.86899024e-02,
        # 7.06622985e-04] in J mol^-1 K^-1
        ...
        # we got 129.3547287392227 + 54.73729016622342 + 3.144125621155244 + 0
        # = 187.23614452660138 J mol^-1 K^-1
        expected_rotational_entropy = 8.314462145468951 * (
            np.log(
                1
                / g16_output.rotational_symmetry_number
        ...
        # E_tot = E_t + E_r + E_v + E_e
        # we got 3718.4353330073513 + 2478.956888671568 + 31636.50928586775 + 0
        # = 37833.90150754667 J mol^-1
        expected_translational_internal_energy = (
            3 / 2 * 8.314462145468951 * 298.15
        )
        expected_rotational_internal_energy = 8.314462145468951 * 298.15
        ...
        """Values from Goodvibes, as a reference:
           ...
           ********************************************************************
        o co2 -188.444680 0.011776 -188.429325 -188.429327 0.021262 0.021262
        -188.450587 -188.450589
           ********************************************************************
        """
        ...
        # we got [4258.62774713, 4258.62774713, 8328.63418484, 14790.61960677]
        # in J mol^-1
        ...
        # we got 3718.4353330073513 + 2478.956888671568 + 31632.978467909088 +
        # 0 = 37830.37068958801 J mol^-1
        ...
        Structure E ZPE H qh-H T.S T.qh-S G(T) qh-G(T)
           ********************************************************************
        o co2 -188.444680 0.011776 -188.429325 -188.429327 0.024281 0.024281
        -188.453606 -188.453608
           ********************************************************************
        """
        ...
        # In Goodvibes, if no concentration is specified, the default pressure
        # is 1 atmosphere.
        assert np.isclose(
        ...
        # S^qrrho_tot = S_t + S_r + S^qrrho_v + S_e
        # we got 155.93822974452405 + 54.73729016622342 + 3.144125621155244 + 0
        # = 213.81964553190272 J mol^-1 K^-1
        expected_translational_entropy = 8.314462145468951 * (
            np.log(
                (
                    (
        ...
        """Values from Goodvibes, as a reference:
                goodvibes -f 100 -c 0.5 -t 598.15 --qs grimme --bav "conf"
                co2.log
        Structure E ZPE H T.S T.qh-S G(T) qh-G(T)
           ********************************************************************
        o co2 -188.444680 0.011776 -188.424452 0.049327 0.049327 -188.473778
        -188.473779
           ********************************************************************
        """
        ...
        Structure E ZPE H qh-H T.S T.qh-S G(T) qh-G(T)
           ********************************************************************
        o co2 -188.444680 0.011776 -188.429325 -188.431976 0.021262 0.021781
        -188.450587 -188.453757
           ********************************************************************
        """
        ...
        # the cutoff frequency for both entropy and enthalpy is specified as
        # 1000 cm^-1
...
class TestThermochemistryHe:
    """He is used as an example to test the thermochemical properties of
    ...
    def test_thermochemistry_he_qrrho(self, gaussian_he_opt_outfile):
        ...
        Structure E ZPE H qh-H T.S T.qh-S G(T) qh-G(T)
           ********************************************************************
        o He -2.915130 0.000000 -2.910394 -2.910394 0.025951 0.025951 -2.936345
        -2.936345
           ********************************************************************
        """
        assert os.path.exists(gaussian_he_opt_outfile)
...
class TestThermochemistryH2O:
    """Water is used as an example to test the thermochemical properties of
    ...
    def test_thermochemistry_water_qrrho(self, gaussian_mp2_outputfile):
        """Values from Goodvibes, as a reference:
                goodvibes -f 500 -c 2.0 -t 1298.15 -q --bav "conf"
                water_mp2.log
        Structure E ZPE H qh-H T.S T.qh-S G(T) qh-G(T)
           ********************************************************************
        o water_mp2 -76.328992 0.021410 -76.289193 -76.289224 0.098212 0.098221
        -76.387404 -76.387445
           ********************************************************************
        """
        assert os.path.exists(gaussian_mp2_outputfile)
...
class TestThermochemistryEntropyMethod:
    ...
    def test_thermochemistry_grimme_method(self, gaussian_singlet_opt_outfile):
        """Values from Goodvibes, as a reference:
                goodvibes --fs 500 -t 298.15 --qs grimme --bav "conf"
                nhc_neutral_singlet.log
        Structure E ZPE H T.S T.qh-S G(T) qh-G(T)
           ********************************************************************
        o nhc_neutral_singlet -1864.040180 0.284336 -1863.732135 0.082255
        0.078764 -1863.814390 -1863.810899
           ********************************************************************
        """
        assert os.path.exists(gaussian_singlet_opt_outfile)
        g16_output = Gaussian16Output(filename=gaussian_singlet_opt_outfile)
    ...
    def test_thermochemistry_truhlar_method(
        ...
        """Values from Goodvibes, as a reference:
                goodvibes --fs 500 -t 298.15 --qs truhlar --bav "conf"
                nhc_neutral_singlet.log
        Structure E ZPE H T.S T.qh-S G(T) qh-G(T)
           ********************************************************************
        o nhc_neutral_singlet -1864.040180 0.284336 -1863.732135 0.082255
        0.052926 -1863.814390 -1863.785062
           ********************************************************************
        """
        assert os.path.exists(gaussian_singlet_opt_outfile)
        g16_output = Gaussian16Output(filename=gaussian_singlet_opt_outfile)
...
class TestBoltzmannWeightedAverage:
    ...
    def test_thermochemistry_boltzmann_electronic(
        ...
        # where boltzmann factors: b_1 = exp(-deltaE1 * beta), b_2 =
        # exp(-deltaE2 * beta)
        # beta = 1 / (R * T)
...
class TestThermochemistryBatchMode:
    ...
    def test_batch_mode_header_writing(
        ...
        - First CLI call (T=298.15K): Processes 2 files (singlet + triplet)
        with header written once
        - Second CLI call (T=398.15K): Processes 1 file (singlet) with new
        header for different temperature
        - Third CLI call (T=198.15K): Processes 1 file (quintet) with overwrite
        enabled, replacing all previous content
        """
        ...
        # This simulates the batch mode where header should NOT be written
        # again
