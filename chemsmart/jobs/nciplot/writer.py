...
class NCIPLOTInputWriter(InputWriter):
        ...
        if self.job.filenames is None:
            ...
            if number_of_files == 0:
                ...
                for file in self.job.filenames:
                    ...
                    # The runner copies all files to scratch using only their
                    # basename.
                    # For converted files, _promolecular suffix is added to the
                    # basename.
                    # We must use basename here to find files in scratch
                    # directory.
    ...
    def _write_fragments(self, f):
        ...
        # defining a different fragment. The same atom should appear only in
        # one
        # of the defined fragments. If the combined list of all the atoms
        # inside
        # a FRAGMENT environment is not exhaustive, then the remaining atoms
        # are
        # not considered. Inside a FRAGMENT environment, atoms can be defined
        # by
        ...
        # than 95% of the density coming from only one fragment), then the
        # value
