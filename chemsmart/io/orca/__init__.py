class ORCARefs:
    ...
    This class provides comprehensive reference lists of ORCA input keywords,
    blocks,
    functionals, basis sets, and other computational parameters. Used for
    validating
    ...
    ORCA_BLOCKS = [
        ...
        # The goal of the ICE-CI is to provide compact wavefunction(s) (e.g.
        # one or several states) close to
        # the full-CI limit at a small fraction of the computational cost.
        # However, ICE-CI itself is not designed
    ...
    ORCA_FUNCTIONALS = [
        ## % method
        # Choices for "Functional". If no reference is given, look further
        # below for the references for individual
        ...
        # and Goerigk (SCS fit gave SOS version; SOS only applies to the CIS(D)
        # component) [206]
    ...
    ORCA_STANDARD_BASIS_SETS_KARLSRUHE_DEF2 = [
        ...
        # and automatically load Stuttgart-Dresden effective core potentials
        # for elements Rb-Rn.
        # ***************************************
        ...
        # Note that this is quite similar to the older (“def ”) TZVPP for the
        # main group elements and TZVP for hydrogen.
    ...
    ORCA_ANO_BASIS_SETS = [
        ...
        # missing) primitives.These are very accurate basis sets that are
        # significantly better than the cc-pVnZ
        # counterparts for the same number of basis functions (but much larger
        # number of primitives of course).
        ...
        # Almost as expensive as the next higher basis set. In fact,
        # aug-ANO-pVnZ = ANO-pV(n + 1)Z with the highest
    ...
    ORCA_EXTRAPOLATION_BASIS = [
        ...
        # aug-ano, def2; if omitted “cc-pVnZ” is used) for cardinal numbers n,m
        # (n<m=2,3,4,5),
        # e.g. Extrapolate(2/3,cc) extrapolates the SCF, MP2 and MDCI energies
        # to the basis set limit.
        # “core” refers to basis sets with core correlation function. frozen
        # core approximation turned off (default).
        # This setting can be overridden in the “methods” block if one just
        # wants to use the basis set with core
        # correlation functions (steep primitives) but without unfreezing the
        # core electrons.
        "Extrapolate(n,bas)",  # Calculate the first n-energies for member of the basis set family basis,
        # e.g. Extrapolate(3) is doing calculations with cc-pVDZ, cc-pVTZ and
        # cc-pVQZ.
        "ExtrapolateEP2(n/m,bas,[method,method-details])",  # Similar: performs SCF, MP2 and MDCI calculations.
        # The higher basis set can only be done with DLPNO-CCSD(T) or MP2
        # methods and then used to extrapolate the MDCI
        ...
        # method is optional and can be either MP2 or DLPNO-CCSD(T), the latter
        # being the default.
        # In case method is DLPNO-CCSD(T), in method-details option one can ask
        # for LoosePNO, NormalPNO or TightPNO.
    ]
    ...
    ORCA_AUXILIARY_COULOMB_BASIS_SETS = [
        ...
        # Consists of the decontracted def2/J up to Kr and of our own auxiliary
        # basis sets for the rest of the periodic
        # table.Appropriate for use in DKH or ZORA calculations with the
        # recontracted versions of the all-electron
        # def2 basis sets (up to Kr) and the SARC basis sets for the heavier
        # elements.
    ...
    def orca_blocks(self):
        """
        Get list of supported ORCA input blocks in lowercase.

        Returns:
            list: All ORCA input block names converted to lowercase
        """
    ...
    def orca_functionals(self):
        """
        Get list of supported ORCA functionals in lowercase.

        Returns:
            list: All ORCA functional names converted to lowercase
        """
    ...
    def orca_ab_initio(self):
        """
        Get list of supported ORCA ab initio methods in lowercase.

        Returns:
            list: All ORCA ab initio method names converted to lowercase
        """
    ...
    def orca_basis_pople(self):
        """
        Get list of Pople-style basis sets in lowercase.

        Returns:
            list: All Pople-style basis set names converted to lowercase
        """
    ...
    def orca_basis_with_polarization(self):
        ...
        Generates combinations of Pople-style basis sets with polarization
        functions.
        For 6-31G basis sets, excludes (3df) and (3df,3pd) polarizations.
        For 6-311G basis sets, includes all polarization combinations.

        Returns:
            list: All Pople basis sets with polarization functions
        """
    ...
    def orca_basis_karlsruhe_def2(self):
        """
        Get list of Karlsruhe def2 basis sets in lowercase.

        Returns:
            list: All def2 basis set names converted to lowercase
        """
        return [
            basis.lower()
    ...
    def orca_basis_karlsruhe_def(self):
        """
        Get list of older Karlsruhe def basis sets in lowercase.

        Returns:
            list: All def basis set names converted to lowercase
        """
        return [
            basis.lower()
    ...
    def orca_basis_karlsruhe_minimally_augmented(self):
        """
        Get diffuse def2 basis sets with minimal augmentation.

        Minimally augmented def2 basis sets with diffuse s and p functions
        according to Truhlar. Recommended for general use.

        Returns:
            list: All minimally augmented def2 basis sets with 'ma-' prefix
        """
    ...
    def orca_basis_karlsruhe_rappoport_property(self):
        """
        Get Rappoport property-optimized diffuse def2 basis sets.

        Augmented def2 basis sets by diffuse functions according to Rappoport
        et al. for improved property calculations.

        Returns:
            list: def2 basis sets with 'D' suffix for diffuse functions
        """
    ...
    def orca_basis_karlsruhe_Dirac_Fock_ECPs(self):
        """
        Get Karlsruhe basis sets with Dirac-Fock ECPs.
        ...
        Dirac-Fock ECPs (dhf-ECP) as opposed to the Wood-Boring ones
        (def2-ECP).
    ...
    def orca_basis_relativistic_DKH_ZORA(self):
        """
        Get relativistically recontracted Karlsruhe basis sets.
        ...
        def2 exponents but have only one contracted function per angular
        momentum
        with contraction coefficients suitable for scalar relativistic
        Hamiltonians.

        Returns:
            list: def2 basis sets with DKH- or ZORA- prefix for relativistic
            calculations
        """
    ...
    def orca_basis_relativistic_DKH_ZORA_ma(self):
        ...
        Returns:
            list: DKH and ZORA basis sets with ma- prefix for minimal
            augmentation
        """
    ...
    def orca_basis_x2c(self):
        """
        Get all-electron Karlsruhe basis sets for exact two-component methods.

        All-electron Karlsruhe basis sets up to Rn for exact two-component
        methods (X2C).

        Returns:
            list: def basis sets with x2c- prefix and 'all' suffix
        """
        orca_basis_x2c = []
    ...
    def orca_basis_x2c_2c(self):
        """The “-2c” variants are intended for two-component calculations
    ...
    def orca_basis_x2c_nmr(self):
        """The “-s” variants are augmented with additional tight functions for
        NMR shielding calculations. [5].
        """
        orca_basis_x2c_nmr = []
        for basis in self.orca_basis_x2c:
    ...
    def orca_basis_SARC(self):
        """
        Get segmented all-electron relativistically contracted basis sets.

        For use with the DKH2 and ZORA Hamiltonians. Available for elements
        beyond Krypton. Note: SARC/J is the general-purpose Coulomb-fitting
        auxiliary for all SARC orbital basis sets.

        Returns:
            list: SARC basis set names converted to lowercase
        """
        orca_basis_SARC = [
            "SARC-DKH-TZVP",
            "SARC-DKH-TZVPP",
    ...
    def orca_basis_SARC2(self):
        ...
        Suitable for accurate calculations using correlated wavefunction
        methods.
        ...
        Returns:
            list: SARC2 basis set names converted to lowercase
        """
        orca_basis_SARC2 = [
            "SARC2-DKH-QZVP",
            "SARC2-ZORA-QZVP",
        ...
        for basis in self.ORCA_PARTIALLY_AUGMENTED_CC_BASIS_SETS:
            if basis.startswith("apr"):
                ...
                for i in ["D", "T", "Q"]:
                    ...
                    # (n = D, T, Q): spd (D), spdf (T), spdfg (Q) on Li-Ca for
                    # jul
                    # same as jun-, may-, and apr- for n = D, T, and Q,
                    # respectively for maug
    ...
    def orca_auxiliary_basis_coulomb_exchange(self):
        """Auxiliary basis sets for simultaneously fitting Coulomb and
        exchange.

        Fitting basis sets developed by Weigend for fitting simultaneously
        Coulomb and exchange energies.
        They are quite large and accurate. They fit SCF energies very well but
        even if they are large they do not fit
    ...
    def orca_all_methods(self):
        """
        Get complete list of all ORCA computational methods.

        Combines all DFT functionals and ab initio methods supported by ORCA.

        Returns:
            list: Combined list of functionals and ab initio methods
        """
    ...
    def orca_all_solvents(self):
        """
        Get complete list of all ORCA solvents.

        Combines full solvent names and common short names supported by ORCA
        for solvation calculations.

        Returns:
            list: Combined list of full and abbreviated solvent names
        """
    ...
    def orca_density_options(self):
        """
        Get ORCA density calculation options.

        Returns:
            list: Available density calculation options in lowercase
        """
