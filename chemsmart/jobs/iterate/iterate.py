...
class BasePreprocessor:
    """Base preprocessor class with shared logic for Skeleton and Substituent
    preprocessors.
    """
    ...
    def _has_available_bonding_position(self) -> bool:
        ...
        for neighbor in graph.neighbors(self.link_index):
            ...
            # Default to 1.0 if bond_order is missing (shouldn't happen with
            # to_graph)
    ...
    def detect_substituent(self) -> list[int]:
        ...
        # For each neighbor, calculate the size of the component if bond is
        # broken.
    ...
    def _run_auto_detect(self) -> Molecule:
        """
        Run auto-detection mode to find and remove the smallest group at link
        position.
    ...
    def _get_fallback_indices(self) -> list[int]:
        """
        Get indices to use for new link index calculation if run() wasn't
        called.
...
class SkeletonPreprocessor(BasePreprocessor):
    """Preprocessor to prepare skeleton molecule by removing substituent at
    link position.

    This class handles two scenarios:
    1. User provides skeleton_indices: Keep only atoms at specified indices
    2. User doesn't provide skeleton_indices: Auto-detect and remove
    substituent at link position
    """
    ...
    def __init__(
        ...
        link_index : int
            Index of the link atom (1-based, will be converted to 0-based
            internally)
    ...
    def run(self) -> Molecule:
        ...
        if self.skeleton_indices is not None:
            ...
            if len(substituent_branches) == 0:
                # No substituent to remove at this link_index, return molecule
                # as-is
                self._final_indices = list(range(len(self.molecule)))
                return self.molecule
            elif len(substituent_branches) == 1:
                # Exactly one substituent branch - remove only this branch
                # Keep all other atoms (including substituents on other link
                # atoms)
                ...
                # Multiple substituent branches - ambiguous, fall back to
                # auto-detection
                logger.warning(
    ...
    def _find_non_skeleton_branches(self) -> list[list[int]]:
        """
        Find all branches from link_index that don't contain any skeleton
        atoms.
    ...
    def _dfs_collect_branch(
        ...
        """
        Collect all atoms in a branch using DFS, excluding the starting point's
        parent.
...
class SubstituentPreprocessor(BasePreprocessor):
    """Preprocessor to prepare substituent molecule by removing atom/group at
    link position if needed.

    This class checks if the link atom has an available bonding position.
    If not, it auto-detects and removes the smallest substituent group at the
    link position.

    Unlike SkeletonPreprocessor, SubstituentPreprocessor does not have a
    "skeleton_indices" concept.
...
class IterateAnalyzer:
    """Analyzer for a pair of skeleton and substituent in Iterate job, find the
    optimal position to place the substituent on skeleton.
    """
    ...
    def __init__(
        ...
        skeleton_link_index : int
            Index of the link atom in skeleton (1-based, will be converted to
            0-based internally)
        substituent_link_index : int
            Index of the link atom in substituent (1-based, will be converted
    ...
    def run(self) -> Optional[Molecule]:
        ...
        -------
        Molecule or None
            Combined molecule with skeleton and optimally positioned
            substituent,
    ...
    def _update_molecule_positions(
        ...
        """
        Create a new Molecule with updated positions, preserving other
        attributes.
    ...
    def _calc_relative_coords(mol: np.ndarray, base_index: int) -> np.ndarray:
        """
        Calculate relative coordinates of all atoms with respect to a base
        atom.
        ...
        np.ndarray
            ...
            relative to the base atom. The base atom will have coordinates (0,
            0, 0).
        """
        n = mol.shape[0]
    ...
    def _find_optimal_position(
        ...
        """
        Find the optimal position for a substituent molecule (sub) to be
        attached to a
        skeleton molecule, such that the total distance sum is maximized while
        avoiding
        atomic overlaps.

        The optimization is performed by finding the optimal position for the
        link atom
        in sub (sub_link), and then placing all other atoms in sub according to
        their
        ...
        ----------
        skeleton_coord : np.ndarray
            Skeleton molecule, shape (n, 4), each row is [atomic_number, x, y,
            z]
        sub_coord : np.ndarray
            Substituent molecule, shape (m, 4), each row is [atomic_number, x,
            y, z]
        skeleton_link_index : int
        ...
        3. Distance(sub_i, skeleton_j) >= min_dist for all i, j (inequality
        constraints)

        Objective
        ---------
        Maximize: sum of all pairwise distances between sub atoms and skeleton
        atoms
        """
        ...
        # Calculate bond distance for equality constraint (sub_link to
        # skeleton_link)
    ...
    def _fibonacci_sphere(num_points: int) -> np.ndarray:
        """
        Generate nearly uniformly distributed unit vectors on a sphere using
        ...
        -------
        np.ndarray
            Array of shape (num_points, 3) containing the Cartesian coordinates
            (x, y, z)
    ...
    def _optimize_lagrange(
        ...
        sub_link_index : int
            Index of the link atom in sub (0-based)
        ineq_mask : np.ndarray
            Mask to exclude link-link pair from inequality constraints, shape
            (m, n)
        sphere_direction_samples_num : int
            Number of points to sample on the unit sphere
        axial_rotations_sample_num : int
        ...
        def objective(opt_vars):
            ...
            We want to find a conformation where the substituent is as "far
            away"
            from the skeleton as possible to avoid crowding, while satisfying
            constraints.
            Mathematically, we maximize the sum of all pairwise distances
            between
            substituent atoms and skeleton atoms.
            Since scipy.optimize.minimize finds the minimum, we return the
            negative sum.
        ...
        def eq_constraint(opt_vars):
            ...
            substituent's link atom is exactly equal to the target bond
            distance.
        ...
        def ineq_constraint(opt_vars):
            ...
            the distance must be greater than or equal to the minimum allowed
            distance
        ...
        # Initial guess
        # 1. Position: Find optimal initial position for sub_link on the
        # constraint sphere
        # We run a preliminary optimization to place the sub_link atom as far
        # as possible
        ...
        def objective_maximize_separation(link_pos_candidate):
            """
            Objective function for preliminary optimization: Maximize
            separation.

            Calculates the negative sum of distances between the candidate link
            position
            and all skeleton atoms. Minimizing this value maximizes the total
            distance,
            effectively pushing the substituent's anchor point away from the
            skeleton bulk.
        ...
        def constraint_bond_length(link_pos_candidate):
            ...
            Ensures the distance between the candidate link position and the
            skeleton's
        ...
        # sphere_direction_samples_num and axial_rotations_sample_num are
        # passed as arguments
        ...
        # Determine the principal axis of the substituent relative to its link
        # atom.
        ...
        for i in range(sphere_direction_samples_num):
            ...
            # Find rotation that aligns the substituent's principal axis to the
            # target vector
            ...
            for j in range(axial_rotations_sample_num):
                ...
                try:
                    ...
                    # Prioritize successful runs with lower objective value
                    # (max distance)
                    if opt_result.success:
                        ...
                        # If optimization failed, but we don't have any success
                        # yet, keep it as fallback
