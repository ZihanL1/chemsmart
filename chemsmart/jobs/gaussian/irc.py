...
class GaussianIRCJob(GaussianJob):
    ...
    def _job_is_complete(self):
        ...
        Returns:
            bool: True if the required IRC calculations are complete, False
            otherwise.
        """
        if self.settings.direction == "forward":
    ...
    def backup_files(self, backup_chk=False):
        """
        Create backup copies of IRC input and output files based on direction
        setting.

        Backs up files from the required IRC calculations based on the
        direction parameter:
