...
class FileOrganizer:
    ...
    Notes: (1) empty rows are okay and will be skipped;
           ...
           (3) no ".log" (or other) extension is needed for each column in
           Excel document.

    The final result will be that inside the current directory, a number of
    subdirectories
    named in column A will be created and the files in column C will be named
    ...
    def clean_data(self):
        """
        Clean data from Excel file by removing empty structure names in second
        column.
    ...
    def create_directories(self, folders):
        ...
        provided folder names. Only creates directories that don't already
        exist.
