...
class Server(RegistryMixin):
    ...
    def scheduler(self):
        ...
        Returns:
            str or None: Scheduler type (e.g., 'slurm', 'pbs') or None if not
            set.
        """
    ...
    def _get_submit_command(self):
        ...
        Returns:
            str or None: Submission command for the scheduler or None if
            unknown.
        """
    ...
    def from_scheduler_type(cls):
        ...
        Returns:
            Server: Server instance for the detected scheduler (or local
            fallback), typically a YamlServerSettings.

        Raises:
            ValueError: If no server class is defined for the detected
            scheduler type.
        """
    ...
    def detect_server_scheduler():
        ...
        Returns:
            str: The detected scheduler type (e.g., SLURM, PBS, LSF, SGE,
            HTCondor)
    ...
    def _from_server_name(cls, server_name):
        ...
        Args:
            server_name (str): Name of the server (with or without .yaml
            extension).
    ...
    def _from_servers_manager(cls, manager):
        ...
        Returns:
            Server or None: Server if loaded; None only when the file is
            missing.
    ...
    def submit(self, job, test=False, cli_args=None, **kwargs):
        ...
        Args:
            ...
            test (bool): If True, only creates scripts without actual
            submission.
                Defaults to False.
            cli_args: Command line arguments for the job.
            **kwargs: Additional submission parameters.
        """
        # First check that the job to be submitted is not already
        # queued/running
...
class YamlServerSettings(Server):
    ...
    Attributes:
        ...
        kwargs (dict): Parsed configuration values from the YAML under
        "SERVER".
